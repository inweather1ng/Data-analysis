...
tab1,tab2,tab3,tab4,tab5,tab6,tab7,tab8 = st.tabs(["Analysis of all variables","Inflow Analysis","Outflow analysis","Lake level Analysis","Yearly Outflow","Yearly Inflow","Reservoir minimum and maximum","Precipitation yearly average"])
with tab1:
...
with tab2:
    ...
    st.subheader("Average inflow trends")
    sns.lineplot(
    ...
    theax.set_title("Inflow graph over time")
    theax.set_xlabel("months")
    theax.set_ylabel("Inflow trend")
    theax.grid(True)
    st.pyplot(figure)
with tab3:
    ...
    theaxis.set_title("Evaporation over time")
    theaxis.set_xlabel("Month")
    theaxis.set_ylabel("the Evaporation average")
    theaxis.grid(True)
    st.pyplot(figure2)
with tab4:
    st.header("Swe patterns over time")
    swe_monthly = df.groupby('month')['SWE_Value'].mean().reset_index()
    swe_monthly.rename(columns={'SWE_Value':"swe average"},inplace=True)
    figure3,axis3 = plt.subplots()
    st.subheader("Swe averages")
with tab5:
    ...
    axis4.set_ylabel("Total Outflow Volume")
...
with tab7:
    ...
    st.link_button('master data','')
with tab8:
    st.header("Precipitation averages YOY")
    precipitation_averages = df.groupby("year")['Precip'].mean().reset_index()
